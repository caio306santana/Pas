import { Controller, Get, Param, Body, Put } from '@nestjs/common';
import { TenantService } from './tenant.service';
import { ApiTags, ApiOperation } from '@nestjs/swagger';

@ApiTags('Tenants')
@Controller('tenants')
export class TenantController {
  constructor(private readonly tenantService: TenantService) {}

  @Get(':slug')
  @ApiOperation({ summary: 'Get tenant information by its slug' })
  async findBySlug(@Param('slug') slug: string) {
    return this.tenantService.findBySlug(slug);
  }

  @Get(':tenantId/config')
  @ApiOperation({ summary: 'Get configurations for a specific tenant' })
  async getConfigs(@Param('tenantId') tenantId: string) {
    return this.tenantService.getConfigs(tenantId);
  }

  @Put(':tenantId/config')
  @ApiOperation({ summary: 'Update configurations for a specific tenant' })
  async updateConfigs(
    @Param('tenantId') tenantId: string,
    @Body() configData: any,
  ) {
    return this.tenantService.updateConfigs(tenantId, configData);
  }
}
