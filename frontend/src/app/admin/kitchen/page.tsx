              ...
              return (
                <article key={order.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-sm">
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-xl bg-primary px-4 py-3.5 text-xs font-black text-white shadow-lg shadow-primary/10 hover:brightness-110 disabled:opacity-60"
                  >
                    <CheckCircle2 className="mr-2 inline h-4 w-4" />
                    {updatingId === order.id ? 'Atualizando...' : next === 'READY' ? 'Marcar como pronto' : 'Avancar etapa'}
                  </button>
                </article>
              );
            })}
